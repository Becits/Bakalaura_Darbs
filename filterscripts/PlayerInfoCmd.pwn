	...
	if(!strcmp(cmd,"playerinfo",true))
	{
		...
		for(new i=0; i<MAX_PLAYERS; i++)
		{
			if(IsPlayerConnected(i))
			{
			    if(n == 0)
				{
					print("ID: Lietotajvards:    IP:               Ping: RCON: Transportliidzeklis:          Statuss:");
					print("--- -------------     ---------------   ----- ----- --------                      --------");
					n++;
				}
				...
				//------------------ Zinas paraadiishana cmd -----------------//
			    printf("%i%s %17s %17s %i %s %5s %17s %s", i, Tab_PlayerID_Length, PlayerName, PlayerIp, PlayerPing, Tab_PlayerPing_Length, PlayerAdmin, PlayerVehicle[PlayerVehicleModelID], PlayerState);
			}
		}
		if(n == 0) { print("Serveri nav neviena speletaja!"); }
	}
	return 1;
}
